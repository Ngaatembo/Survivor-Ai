...
function websitePrice(opp: Opportunity): { price: number; rationale: string } {
  ...
  if (/e-?commerce|online store|shop|payment gateway|custom app|advanced booking|admin panel/.test(text)) {
    return {
      price: WEBSITE_PRICE_LADDER.custom,
      rationale: 'Configured NWT Dev pricing ladder: $450 for custom, e-commerce, advanced booking, payment or admin-panel scope.',
    };
  }

  if (/booking|reservation|restaurant|hotel|guest house|car rental|multi-page|5-page|cms|dashboard/.test(text)) {
    return {
      price: WEBSITE_PRICE_LADDER.advanced,
      rationale: 'Configured NWT Dev pricing ladder: $350 for an advanced business website with richer functionality or larger scope.',
    };
  }

  if (/business|company|contractor|service|4-page|four-page|5-page/.test(text)) {
    return {
      price: WEBSITE_PRICE_LADDER.standard,
      rationale: 'Configured NWT Dev pricing ladder: $250 for a standard business website.',
    };
  }

  return {
    price: WEBSITE_PRICE_LADDER.starter,
    rationale: 'Configured NWT Dev pricing ladder: $150 starter price for a smaller/basic web presence.',
  };
}

function estimatePrice(opp: Opportunity): { price: number; rationale: string } {
  // Never derive website quotes from modeled monthly revenue. That old
  // heuristic produced unrealistic $15/$25-style quotes.
  if (isWebsiteOpportunity(opp)) return websitePrice(opp);

  // Non-website opportunities still need live market-price research before
  // being presented as a verified quote. This is only a clearly labelled
  // model estimate until that evidence is available.
  ...
  return {
    price,
    rationale: 'Model-based estimate for a non-website service (
...
export function generateBusinessModel(opp: Opportunity, memory: MemoryEntry[], now: number = Date.now()): BusinessModel {
 ...
 + price + '). Not a verified market rate; live market-price research should replace it before quoting.',
  };
}
